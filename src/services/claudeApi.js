...
export function findBestMatch(input, dataset) {
...
// Preserved to keep the older local report parser available without affecting demo report mode.
// eslint-disable-next-line no-unused-vars
function buildLegacyReportResponse(fileData) {
  if (!fileData) {
  ...
  if (fileData.type === "pdf") {
    return [
      "📋 REPORT SUMMARY",
      `PDF file received: ${fileData.name || "Unknown file"}`,
      "",
      "🔍 KEY FINDINGS",
      "Your browser cannot extract text from PDF files without a backend. The filename and structure have been noted.",
      "",
      "✅ NEXT STEPS",
      "For full PDF analysis, either convert your PDF to a text file (.txt) first, or paste the key values from your report directly into the chat.",
      "",
      "⚠️ This is a local demo. Always consult a qualified doctor for medical interpretation.",
    ].join("\n");
  }
...
const DEMO_REPORT = {
  patientName: "Miss. SPANDANA",
  age: "22 yrs",
  sex: "Female",
  lab: "Sri Guru Diagnostic Laboratory",
  reportNo: "EASY022613060",
  date: "27-Feb-2026",
  test: "Random Blood Sugar (RBS)",
  result: "133.8 mg/dL",
  referenceRange: "80-160 mg/dL",
  status: "NORMAL",
  method: "GOD/POD",
  adaZones: {
    normal: "80-139 mg/dL",
...
function buildReportResponse(userQuestion) {
  const question = normalizeText(userQuestion);
  const overviewIntent = !question || matchesAny(question, ["explain", "summary", "overview", "read"]);
  const sugarIntent = matchesAny(question, ["sugar", "glucose", "rbs"]);
  const diabetesIntent = matchesAny(question, ["pre diabetic", "prediabetic", "diabetic", "diabetes"]);
  const normalIntent = matchesAny(question, ["normal", "abnormal", "result", "ok"]);
  const patientIntent = matchesAny(question, ["patient", "name", "age", "details"]);
  const causeIntent = matchesAny(question, ["why", "cause", "reason", "high"]);
  const adviceIntent = matchesAny(question, ["next steps", "advice", "recommend", "what should"]);

  const reportTable = [
    "| Field | Value |",
    "| --- | --- |",
    `| Patient | ${DEMO_REPORT.patientName} |`,
    `| Age / Sex | ${DEMO_REPORT.age}, ${DEMO_REPORT.sex} |`,
    `| Lab | ${DEMO_REPORT.lab} |`,
    `| Report No | ${DEMO_REPORT.reportNo} |`,
    `| Date | ${DEMO_REPORT.date} |`,
    `| Test | ${DEMO_REPORT.test} |`,
    `| Result | ${DEMO_REPORT.result} |`,
    `| Reference Range | ${DEMO_REPORT.referenceRange} |`,
    `| Status | ${DEMO_REPORT.status} |`,
    `| Method | ${DEMO_REPORT.method} |`,
  ].join("\n");

  const adaTable = [
    "| ADA 2015 Zone | Range |",
    "| --- | --- |",
    `| Normal | ${DEMO_REPORT.adaZones.normal} |`,
    `| Pre-Diabetes | ${DEMO_REPORT.adaZones.preDiabetes} |`,
    `| Diabetes | ${DEMO_REPORT.adaZones.diabetes} |`,
  ].join("\n");
  ...
  if (overviewIntent) {
    return [
      ...
      "",
      "### Interpretation",
      `The reported **Random Blood Sugar (RBS)** value is **${DEMO_REPORT.result}**, which falls inside the lab reference range of **${DEMO_REPORT.referenceRange}**.`,
      `Based on the report, the status is **${DEMO_REPORT.status}**, which means this single reading is not showing diabetes or pre-diabetes.`,
      "",
      "### ADA 2015 Classification",
      adaTable,
      "",
      "### Quick Take",
      "- Blood sugar is within the normal range for this report.",
      "- The value is below the ADA pre-diabetes threshold of 140 mg/dL.",
      "- No diabetic range is seen in this hardcoded demo report.",
      "",
      "_Demo note: report mode is intentionally answering from one hardcoded blood report for the presentation._",
    ].join("\n");
  }

  if (sugarIntent) {
    return [
      "## RBS Value Explanation",
      "",
      `The **Random Blood Sugar (RBS)** result is **${DEMO_REPORT.result}**.`,
      `The lab reference range is **${DEMO_REPORT.referenceRange}**, so this value is within the expected range.`,
      "",
      "| Item | Reading | Meaning |",
      "| --- | --- | --- |",
      `| RBS | ${DEMO_REPORT.result} | Normal for this report |`,
      "| Pre-Diabetes Cutoff | 140 mg/dL and above | Not reached |",
      "| Diabetes Cutoff | 200 mg/dL and above | Not reached |",
      "",
      "This means her random sugar reading is normal in this report and does not suggest diabetes based on this value alone.",
    ].join("\n");
  }
  ...
  if (diabetesIntent) {
    return [
      "## Diabetes Check",
      "",
      "She is **not diabetic** based on this hardcoded report.",
      `Her RBS value is **${DEMO_REPORT.result}**, which is below the ADA 2015 **pre-diabetes** range starting at **140 mg/dL** and well below the **diabetes** range of **200 mg/dL or higher**.`,
      "",
      "### Conclusion",
      "- Not in the diabetes range",
      "- Not in the pre-diabetes range",
      "- Current report status: **NORMAL**",
    ].join("\n");
  }
  ...
  if (normalIntent) {
    return [
      "## Result Status",
      "",
      "Yes, the result is **normal**.",
      `The reported blood sugar is **${DEMO_REPORT.result}**, and the stated reference range is **${DEMO_REPORT.referenceRange}**.`,
      `Because **133.8 mg/dL** falls within that range, the report marks it as **${DEMO_REPORT.status}**.`,
    ].join("\n");
  }
  ...
  if (patientIntent) {
    return [
      ...
      "| Field | Information |",
      "| --- | --- |",
      `| Patient Name | ${DEMO_REPORT.patientName} |`,
      `| Age | ${DEMO_REPORT.age} |`,
      `| Sex | ${DEMO_REPORT.sex} |`,
      `| Lab | ${DEMO_REPORT.lab} |`,
      `| Report Number | ${DEMO_REPORT.reportNo} |`,
      `| Report Date | ${DEMO_REPORT.date} |`,
    ].join("\n");
  }

  if (causeIntent) {
    return [
      "## Why 133.8 mg/dL Is Still Normal",
      "",
      "A random blood sugar test is checked at whatever time the sample was taken, not necessarily after fasting.",
      `Her value is **${DEMO_REPORT.result}**, which stays inside both the lab reference range of **${DEMO_REPORT.referenceRange}** and the ADA normal zone of **${DEMO_REPORT.adaZones.normal}**.`,
      "",
      "### Reason",
      "- Random sugar can be a little higher than fasting sugar because meals, snacks, and timing affect it.",
      "- A value of 133.8 mg/dL is still below the pre-diabetes cutoff of 140 mg/dL.",
      "- That is why the report classifies the result as **NORMAL** rather than high or abnormal.",
    ].join("\n");
  }
  ...
  if (adviceIntent) {
    return [
      ...
      "",
      "Her current blood sugar result is normal, so this report is reassuring.",
      "",
      "### Healthy Advice",
      "- Continue balanced meals with controlled sugar intake.",
      "- Stay active with regular walking or exercise.",
      "- Keep good sleep and hydration habits.",
      "- Repeat testing only if a doctor advises it or if symptoms like excess thirst, frequent urination, or unexplained weight loss appear.",
      "- Keep regular routine health checkups if there is a family history of diabetes.",
    ].join("\n");
  }
  ...
  return [
    ...
    "",
    "This demo report analyser is using one hardcoded blood report. You can ask things like:",
    "",
    "- `Explain the report`",
    "- `What is the RBS value?`",
    "- `Is she diabetic?`",
...
export function selectDataset(mode) {
  switch (mode) {
    case "symptom":
...
export default async function sendMessage(messages, systemPrompt, fileData, mode) {
  ...
  if (mode === "report") {
    return {
      reply: buildReportResponse(getLatestUserMessage(messages) || systemPrompt || ""),
      sev: null,
    };
  }
  ...
  if (!dataset) {
    return {
      reply: "Unsupported mode. Please use symptom, qa, mental, or report.",
      sev: null,
    };
  }
  ...
  if (!bestEntry) {
    return {
      reply: FALLBACKS[mode] || "I could not find a helpful local match for that message.",
      sev: null,
    };
  }
